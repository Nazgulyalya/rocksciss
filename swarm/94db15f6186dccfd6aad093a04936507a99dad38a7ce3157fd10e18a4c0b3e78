...
contract RockPaperScissors {
    ...
    uint public betAmount = 0.0001 ether;
    ...
    event GameResult(address indexed player, Choices choice, string result);
    ...
    function resolveGame() private {
        ...
        if (player1Choice == player2Choice) {
            ...
            payWinner(payable(player1), betAmount);
            payWinner(payable(player2), betAmount);
        } else if (
    ...
    function payWinner(address payable player, uint amount) private {
        if (player != address(0)) {
